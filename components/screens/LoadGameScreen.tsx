...
export const LoadGameScreen: React.FC<LoadGameScreenProps> = ({ settings, onContinue, onNewGame, onBack }) => {
  ...
  const t = TRANSLATIONS[settings.language].loadGame;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full w-full p-8 bg-slate-900">
      <div className="bg-slate-800 p-8 rounded-xl shadow-2xl max-w-md w-full border border-slate-700 text-center">
        <h2 className="text-3xl font-black mb-8 text-white tracking-tighter uppercase">{t.title}</h2>
        
        {saveData ? (
          <div className="mb-8 bg-slate-700/50 p-4 rounded-lg border border-slate-600">
             <h3 className="text-xl font-bold text-yellow-400 mb-2">{t.foundSave}</h3>
             <p className="text-slate-300">{t.level}: {saveData.campaignLevel}</p>
             <p className="text-slate-300">{t.gold}: {saveData.gold}</p>
             <p className="text-slate-300">{t.deckSize}: {saveData.masterDeck.length}</p>
             <p className="text-slate-400 text-xs mt-2 italic">{t.saveNote}</p>
          </div>
        ) : (
