...
export const useGameLogic = ({
  ...
  const [selectedPiecePos, setSelectedPiecePos] = useState<Position | null>(
    null
  );
  ...
  const [lastEnemyMoveType, setLastEnemyMoveType] = useState<PieceType | null>(
    null
  );
  const [lastPlayerSpawnedType, setLastPlayerSpawnedType] =
    useState<PieceType | null>(null); // For Mirror Mage
  // Changed to useRef to avoid stale closure issues in setTimeout callbacks
  const tempTileEffects = useRef<
    {
      pos: Position;
      originalEffect: TileEffect;
      variant: TileEffect;
      turnsLeft: number;
    }[]
  >([]);
  ...
  // Enemy Visual State
  const [selectedEnemyPos, setSelectedEnemyPos] = useState<Position | null>(
    null
  );
  ...
  const initGame = (
    ...
    if (!campaignMode) {
      ...
      const starterRelics = relics.filter((r) =>
        [
          RelicType.START_PAWN,
          RelicType.START_ROOK,
          RelicType.START_KNIGHT,
          RelicType.START_BISHOP,
          RelicType.START_QUEEN,
        ].includes(r.type)
      );
    ...
    if (campaignMode && campaignDeck) {
      ...
      gameDeck = Array.from({ length: 20 }).map(() => {
        const template =
          deckTemplate[Math.floor(Math.random() * deckTemplate.length)];
  ...
  const spawnRelicPiece = (
    ...
    baseRows.forEach((r) => {
      for (let c = 0; c < size; c++) {
        ...
        if (
          !cell.piece &&
          cell.tileEffect !== TileEffect.WALL &&
          cell.tileEffect !== TileEffect.HOLE
        ) {
  ...
  const executeMove = (from: Position, to: Position) => {
      ...
      while (queue.length > 0) {
        ...
        if (curr.col > 0) {
          ...
          if (!checked.has(leftKey)) {
            ...
            if (
              leftP &&
              leftP.type === PieceType.ELEPHANT &&
              leftP.side === piece.side
            ) {
              checked.add(leftKey);
              queue.push(leftPos);
            }
        ...
        if (curr.col < size - 1) {
          ...
          if (!checked.has(rightKey)) {
            ...
            if (
              rightP &&
              rightP.type === PieceType.ELEPHANT &&
              rightP.side === piece.side
            ) {
      ...
      if (swarm.length >= 3) {
        swarm.forEach((pos) => {
          ...
          if (
            ...
            else if (targetCell.piece && targetCell.piece.side !== piece.side)
              canSwarmMove = true;
            ...
            if (canSwarmMove) {
              ...
              if (targetCell.piece) {
                ...
                const midas = relics.find(
                  (r) => r.type === RelicType.MIDAS_TOUCH
                );
    ...
    if (
      (piece.type === PieceType.PAWN ||
        piece.type === PieceType.ZEBRA ||
        piece.type === PieceType.CHAMPION) &&
      !target &&
      from.col !== to.col &&
      enPassantTarget
    ) {
    ...
    if (destEffect === TileEffect.LAVA && !ignoresTerrain) {
      if (
        !(
          newBoard[to.row][to.col].piece!.immortalTurns &&
          newBoard[to.row][to.col].piece!.immortalTurns! > 0
        )
      ) {
    ...
    if (enemyKilled) {
      ...
      if (necromancy) {
        spawnRelicPiece(
          newBoard,
          Side.WHITE,
          RELIC_LEVEL_REWARDS[Math.min(necromancy.level, 5)]
        );
      }
      ...
      if (activeBoss === BossType.HYDRA) {
        ...
        for (let r = 0; r < enemyHalfLimit; r++) {
          for (let c = 0; c < size; c++) {
            ...
            if (
              !newBoard[r][c].piece &&
              newBoard[r][c].tileEffect === TileEffect.NONE
            ) {
    ...
    if (selfKilled) {
      ...
      if (lastWill) {
        spawnRelicPiece(
          newBoard,
          Side.WHITE,
          RELIC_LEVEL_REWARDS[Math.min(lastWill.level, 5)]
        );
      }
    }
    ...
    newBoard.forEach((row) =>
      row.forEach((cell) => {
        if (cell.piece && cell.piece.side === Side.WHITE) {
          if ((cell.piece.frozenTurns || 0) > 0)
            cell.piece.frozenTurns = (cell.piece.frozenTurns || 0) - 1;
          if ((cell.piece.immortalTurns || 0) > 0)
            cell.piece.immortalTurns = (cell.piece.immortalTurns || 0) - 1;
  ...
  const endPlayerTurn = (
    ...
    setTimeout(
      () => executeEnemyTurn(currentBoard, currentEnPassantTarget),
      800
    );
  };

  const applyBossAbilities = (boardState: Cell[][]) => {
    if (activeBoss === BossType.NONE)
      return { board: boardState, newTiles: [] as Position[] };
    ...
    for (let r = 0; r < size; r++) {
      for (let c = 0; c < size; c++) {
        if (
          ...
          boardState[r][c].tileEffect === TileEffect.NONE
        ) {
    ...
    if (activeBoss === BossType.BLIZZARD_WITCH)
      effectToApply = TileEffect.FROZEN;
  ...
  const executeEnemyTurn = (
    currentBoard: Cell[][],
    playerEnPassantTarget: Position | null
  ) => {
    const { board: boardAfterBoss, newTiles } = applyBossAbilities([
      ...currentBoard,
    ]);
    setBossTiles(newTiles);

    const boardCopy = boardAfterBoss.map((row) =>
      row.map((cell) => ({
        ...cell,
        piece: cell.piece ? { ...cell.piece } : null,
      }))
    ...
    if (activeBoss === BossType.MIRROR_MAGE && lastPlayerSpawnedType) {
      ...
      for (let r = 0; r < enemyHalfLimit; r++) {
        for (let c = 0; c < size; c++) {
          if (
            !boardCopy[r][c].piece &&
            boardCopy[r][c].tileEffect !== TileEffect.WALL
          ) {
            emptySpots.push({ row: r, col: c });
          }
        }
        ...
        if (p?.side === Side.BLACK) {
          if ((p.frozenTurns || 0) > 0) {
            ...
            let moves = getValidMoves(
              ...
              p,
              { row: r, col: c },
              playerEnPassantTarget,
              null
            );
            if (isEnemyMoveLimited) {
              moves = moves.filter(
                (m) => Math.abs(m.row - r) <= 1 && Math.abs(m.col - c) <= 1
              );
            }
            if (moves.length > 0)
    ...
    const passTurn = () => {
      ...
      tempTileEffects.current.forEach((effect) => {
        if (
          boardCopy[effect.pos.row][effect.pos.col].tileEffect ===
          effect.variant
        ) {
          boardCopy[effect.pos.row][effect.pos.col].tileEffect =
    ...
    for (const e of enemies) {
      const kingKill = e.moves.find(
        (m) => boardCopy[m.row][m.col].piece?.type === PieceType.KING
      );
      if (kingKill) {
    ...
    if (!bestMove) {
      ...
      enemies.forEach((e) => {
        e.moves.forEach((m) => {
          ...
          const isEP =
            e.piece.type === PieceType.PAWN &&
            playerEnPassantTarget &&
            m.row === playerEnPassantTarget.row &&
            m.col === playerEnPassantTarget.col;
    ...
    if (!bestMove) {
      ...
      const randomMove =
        randomEnemy.moves[Math.floor(Math.random() * randomEnemy.moves.length)];
    ...
    if (bestMove) {
      ...
      if (
        ...
        playerEnPassantTarget &&
        bestMove.to.row === playerEnPassantTarget.row &&
        bestMove.to.col === playerEnPassantTarget.col
      ) {
        const direction = 1;
      ...
      if (
        movingPiece.type === PieceType.PAWN &&
        Math.abs(bestMove.from.row - bestMove.to.row) === 2
      ) {
        nextEnPassantTarget = {
      ...
      if (
        boardCopy[bestMove.to.row][bestMove.to.col].tileEffect ===
          TileEffect.WALL &&
        (movingPiece.type === PieceType.SHIP ||
          movingPiece.type === PieceType.ELEPHANT)
      ) {
        boardCopy[bestMove.to.row][bestMove.to.col].tileEffect =
          TileEffect.NONE;
      }

      boardCopy[bestMove.to.row][bestMove.to.col].piece = {
        ...movingPiece,
        hasMoved: true,
      };
      ...
      if (effect === TileEffect.LAVA && movingPiece.type !== PieceType.DRAGON) {
        if (
          !(
            boardCopy[bestMove.to.row][bestMove.to.col].piece!.immortalTurns &&
            boardCopy[bestMove.to.row][bestMove.to.col].piece!.immortalTurns! >
              0
          )
        ) {
      ...
      if (
        boardCopy[bestMove.to.row][bestMove.to.col].piece &&
        movingPiece.type === PieceType.PAWN &&
        bestMove.to.row === size - 1
      ) {
        boardCopy[bestMove.to.row][bestMove.to.col].piece!.type =
          PieceType.QUEEN;
        soundManager.playSfx('spawn');
      }
  ...
  const handleSquareDoubleClick = (r: number, c: number) => {
    ...
    if (piece) {
      ...
      setInfoModalContent({
        title: t.tiles[effect].name,
        content: t.tiles[effect].desc,
      });
    }
  };
  ...
  const handleCardClick = (card: Card) => {
    ...
    if (cardsPlayed >= MAX_CARDS_PLAYED_PER_TURN) {
      alert(
        `You have reached the maximum of ${MAX_CARDS_PLAYED_PER_TURN} cards played this turn.`
      );
      return;
    }
    ...
    if (
      card.type.includes('SPAWN') ||
      card.type === CardType.EFFECT_BACK_BASE
    ) {
      ...
      card.type === CardType.EFFECT_SWITCH ||
      card.type === CardType.EFFECT_IMMORTAL ||
      card.type === CardType.EFFECT_MIMIC ||
      card.type === CardType.EFFECT_ASCEND ||
      card.type === CardType.EFFECT_IMMORTAL_LONG ||
      card.type === CardType.EFFECT_AREA_FREEZE
    ) {
  ...
  const playInstantCard = (card: Card) => {
    ...
    if (card.type === CardType.EFFECT_FREEZE) {
      ...
      newBoard.forEach((row, r) =>
        row.forEach((cell, c) => {
          if (cell.piece?.side === Side.WHITE && !cell.piece.trapped)
            friends.push({ row: r, col: c });
        })
      );
      if (friends.length > 0) {
      ...
      if (validSpots.length > 0) {
        ...
        const pieceType =
          deadPieces[Math.floor(Math.random() * deadPieces.length)];
  ...
  const handleCardTargeting = (r: number, c: number, piece: Piece | null) => {
    ...
    if (type.includes('SPAWN')) {
      if (r >= size - 2 && !piece) {
        ...
        else if (type === CardType.SPAWN_ELEPHANT)
          spawnType = PieceType.ELEPHANT;
        else if (type === CardType.SPAWN_CHANCELLOR)
          spawnType = PieceType.CHANCELLOR;
        else if (type === CardType.SPAWN_ARCHBISHOP)
          spawnType = PieceType.ARCHBISHOP;
        ...
        else if (type === CardType.SPAWN_CHAMPION)
          spawnType = PieceType.CHAMPION;
        ...
        if (
          cell.tileEffect === TileEffect.LAVA &&
          spawnType !== PieceType.DRAGON
        ) {
        ...
        for (let row of baseRows) {
          for (let col of cols) {
            if (
              ...
              newBoard[row][col].tileEffect !== TileEffect.WALL &&
              newBoard[row][col].tileEffect !== TileEffect.HOLE
            ) {
              targetPos = { row, col };
              break;
            }
    ...
    if (type === CardType.EFFECT_IMMORTAL_LONG) {
      if (step === 'SELECT_PIECE_1' && piece?.side === Side.WHITE) {
        newBoard[r][c].piece = {
          ...piece,
          immortalTurns: IMMORTAL_LONG_DURATION,
        };
        setBoard(newBoard);
        soundManager.playSfx('spawn');
        consumeCard(selectedCardId);
    ...
    if (type === CardType.EFFECT_SWITCH) {
      if (step === 'SELECT_PIECE_1' && piece?.side === Side.WHITE) {
        ...
        step === 'SELECT_PIECE_2' &&
        piece?.side === Side.WHITE &&
        cardTargetMode.sourcePos
      ) {
        const p1 =
          newBoard[cardTargetMode.sourcePos.row][cardTargetMode.sourcePos.col]
            .piece;
        const p2 = piece;
        if (p1 && p2) {
          newBoard[cardTargetMode.sourcePos.row][
            cardTargetMode.sourcePos.col
          ].piece = p2;
